...
def check(dvc_id, cipher_sym, a, b, serveraddress):
    ...
    r=urllib2.urlopen('http://'+serveraddress+'/emilia-server/index.php/device', urllib.urlencode(datarequest))
    datarespond = json.load(r)
    session_id=datarespond['result']['session_id']

    url="http://"+serveraddress+"/emilia-server/index.php/crypto/?session_id="+session_id+"&plain="+key
    json_obj=urllib2.urlopen(url)
    datacrypto = json.load(json_obj)
    cipher_rsa = datacrypto['result'][0]['cipher']
    ...
    r=urllib2.urlopen('http://'+serveraddress+'/emilia-server/index.php/device', urllib.urlencode(datacheck))
    datacheck = json.load(r)
    status_code=datacheck['result']['status_code']
    ...
    r=urllib2.urlopen('http://'+serveraddress+'/emilia-server/index.php/memberdeviceman', urllib.urlencode(datasession))
    return s_status
...
def randomprime():
    prime=[4507,  4513,  4517,  4519,  4523,  4547,  4549,  4561,  4567,  4583,  4591,  4597, 
...
clear_text = str(c)
key = randomword(len(clear_text))
iv = "InitVector"
cipher_sym = encrypt_aes_128(clear_text, key, iv)
try:
	while True:
		s_status=check(dvc_id, cipher_sym, a, b, serveraddress)
